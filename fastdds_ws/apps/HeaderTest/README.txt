...
In the first one launch: ./DDSHeaderTest publisher (or DDSHeaderTest.exe publisher on windows).
In the second one: ./DDSHeaderTest subscriber (or DDSHeaderTest.exe subscriber on windows).
...
In order to use xml profiles (--env or shorcut -e cli flags):
    ...
    - name it DEFAULT_FASTRTPS_PROFILES.xml and make sure the file is besides the DDSHeaderTest binary.
